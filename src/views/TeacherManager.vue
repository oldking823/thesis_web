...
export default {
  ...
  methods:{
    ...
    page(currentPage){
      ...
      if(_this.ruleForm.value == ''){
        axios.get('http://localhost:8181/teacher/page/'+currentPage+'/'+_this.pageSize).then(function (resp) {
          _this.tableData = resp.data.data.records
          _this.total = resp.data.data.total
        })
      } else {
    ...
    del(row) {
      const _this = this
      this.$confirm('确认删除【'+row.tname+'】吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        axios.delete('http://localhost:8181/teacher/'+row.id).then(function (resp) {
          if(resp.data.code==0){
            _this.$alert('【'+row.tname+'】已删除', '', {
              confirmButtonText: '确定',
              callback: action => {
                location.reload()
              }
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/teacher/page/1/'+_this.pageSize).then(function (resp) {
      _this.tableData = resp.data.data.records
      _this.total = resp.data.data.total
    })
  }
